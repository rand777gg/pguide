...
/**
 * `buildSubjectTree` 的单元测试 —— 它只负责**邻接表 → 嵌套树**这一步纯转换，
 * 不含任何兜底。兜底逻辑在 `services/subject.ts` 的 `loadSubjectTree` 里，
 * 单独测（见 services/__tests__/subject.spec.ts）。
 *
 * 为什么先测这个：后端返回的**不是**嵌套树，而是按父 id 索引的**邻接表**：
 *   { '0': [一级...], '1': [subjectId=1 的子节点...], ... }
 * 老工程在组件里手工拼这个结构，没有测试，改错了也不会有提示。
 *
 * 另外注意 subjectLevel 的类型坑：后端返回的是**字符串** "1"，不是数字。
 */
describe('buildSubjectTree', () => {
  ...
  it('没有 0 号 key 时返回空数组（根节点约定挂在 "0" 下）', () => {
    expect(buildSubjectTree({})).toEqual([])
    expect(
      buildSubjectTree({ '1': [{ subjectId: 1, subjectName: 'x', subjectLevel: '1' }] }),
    ).toEqual([])
  })

  it('子节点引用了不存在的父 id 时不会抛错，只是不会被挂上', () => {
    const adjacency: SubjectAdjacencyList = {
